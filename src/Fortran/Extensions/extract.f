...
c                  WRITE (6,*) 'No GLOBAL PROFILE  PROFILE QC found !'
               ...
               IF ( (GQCMIN .LE. QCPRO .AND. GQCMAX .GE. QCPRO) .OR.
     +              (GQCMIN .EQ. 10) ) THEN
...
C --- Select according to depth
            ...
            NEWREC = 0
            DO K=1,NBREC
              IF ( (PRES(K) .GE. DEPSTA .AND. PRES(K) .LE. DEPEND) .OR.
     +             (DEPSTA .EQ. DEPEND) )        THEN
                NEWREC = NEWREC + 1
                PRES(NEWREC) = PRES(K)
                SAL(NEWREC) = SAL(K)
                TEM(NEWREC) = TEM(K)
...
C --- Open the right NetCDF file
         ...
         IF (NBREC .LE. 15) THEN
            NCID2 = NCOPN (SELOUT(1:LL)//'/modb_0015.nc',
     +                     NCWRITE,RCODE)

         ELSEIF (NBREC .LE. 30) THEN
            NCID2 = NCOPN (SELOUT(1:LL)//'/modb_0030.nc',
     +                     NCWRITE,RCODE)

         ELSEIF (NBREC .LE. 60) THEN
            NCID2 = NCOPN (SELOUT(1:LL)//'/modb_0060.nc',
     +                     NCWRITE,RCODE)

         ELSEIF (NBREC .LE. 100) THEN
            NCID2 = NCOPN (SELOUT(1:LL)//'/modb_0100.nc',
     +                     NCWRITE,RCODE)

         ELSEIF (NBREC .LE. 500) THEN
            NCID2 = NCOPN (SELOUT(1:LL)//'/modb_0500.nc',
     +                     NCWRITE,RCODE)

         ELSEIF (NBREC .LE. 1000) THEN
            NCID2 = NCOPN (SELOUT(1:LL)//'/modb_1000.nc',
     +                     NCWRITE,RCODE)

         ELSEIF (NBREC .LE. 1500) THEN
            NCID2 = NCOPN (SELOUT(1:LL)//'/modb_1500.nc',
     +                     NCWRITE,RCODE)

         ELSEIF (NBREC .LE. 2000) THEN
            NCID2 = NCOPN (SELOUT(1:LL)//'/modb_2000.nc',
     +                     NCWRITE,RCODE)

         ELSEIF (NBREC .LE. 2500) THEN
            NCID2 = NCOPN (SELOUT(1:LL)//'/modb_2500.nc',
     +                     NCWRITE,RCODE)

         ELSEIF (NBREC .LE. 3000) THEN
            NCID2 = NCOPN (SELOUT(1:LL)//'/modb_3000.nc',
     +                     NCWRITE,RCODE)
...
C--------------------------------------------NetCDF-output-code-(end)------
            ...
            ELSEIF (OUTSPC.EQ.3) THEN
              ...
              WRITE (70+MM,FMT=100) TEXT(1)(2:19),DATE,XLAT,XLON,
     +                           QC1,QC2,QC3,QCPRO,NEWREC,DTYPE
              DO K = 1,NEWREC
                 WRITE (70+MM,FMT=101) PRES(K),TEM(K),SAL(K),
     +                              QCP(K),QCT(K),QCS(K)
              ENDDO
...
C           WRITE(6,'(2A)')'Searching in ',PR
            ...
            CALL TOPOS ( XLAT , XLON , LAT1 , LAT2 , LAT3 ,
     +                                 LON1 , LON2 , LON3 )
...
C--------------------------------------------NetCDF-output-code-(end)------
            ...
            ELSEIF (OUTSPC.EQ.3) THEN
              ...
              WRITE (70+MM,FMT=102) TEXT(1)(2:19),DATE,XLAT,XLON,
     +                           QC1,QC2,QC3,QCPRO,NEWREC,DTYPE
              DO K = 1,NEWREC
                 WRITE (70+MM,FMT=101) PRES(K),TEM(K),SAL(K),
     +                              QCP(K),QCT(K),QCS(K)
...
C --- Select on Position, Type and Date  ---
            ...
            DD = DATE / 1 000 000
            MM = ( DATE - DD * 1 000 000 ) / 10 000
            YYYY = DATE - DD * 1 000 000 - MM * 10 000
            MMDD = MM * 100 + DD
...
C --- Loop over profiles in the MED file ---
 ...
 300      CONTINUE
            ...
            READ(70,FMT=1000,END=400) PR,DATE,XLAT,XLON,
     +                    QC1,QC2,QC3,QCPRO,NBREC,DTYPE
            DO K = 1,NBREC
               READ(70,FMT=2000,END=400) PRES(K),TEM(K),SAL(K),
     +                              QCP(K),QCT(K),QCS(K)
            ENDDO
...
302   FORMAT (6x,i8.8,6x,i4.4,5x,a1,i2.2,1x,f5.2,
     +        5x,a1,i3.3,1x,f5.2,7x,
     +        i6,4x,4i1)
...
C --- Read 2nd line
      ...
      READ (LINE,302) DATE,TIME,LAT1,LAT2,LAT3,LON1,LON2,LON3,DEPTH,
     +               QC1,QC2,QC3,QC4
...
302   FORMAT ('*DATE=',i8.8,1x,'TIME=',i4.4,1x,'LAT=',a1,i2.2,1x,f5.2,
     +        1x,'LON=',a1,i3.3,1x,f5.2,1x,
     +        'DEPTH=',i6,1x,'QC=',4i1)
303   FORMAT ('*NB PARAMETERS=',i2,1x,'RECORD LINES=',i5)
304   FORMAT ('*','PRES',1x,'SEA PRESSURE                  ',
     +        'DECIBARS                      ',1x,'def.=','-999.9')
305   FORMAT ('*','TEMP',1x,'SEA TEMPERATURE               ',
     +        'CELSIUS DEGREE                ',1x,'def.=','99.999')
306   FORMAT ('*','PSAL',1x,'PRACTICAL SALINITY            ',
     +        'P.S.U.                        ',1x,'def.=','99.999')
...
C --- WRITING ON FILE 
      ...
      WRITE (IU,302) DATE,TIME,LAT1,LAT2,LAT3,LON1,LON2,LON3,DEPTH,
     +               QC1,QC2,QC3,QC4
...
* netCDF id
      integer  ncid
* dimension ids
      integer  recorddim, leveldim, charlendim, textlendim, dim3dim, dim
     14dim
* variable ids
      integer  latid, lonid, dateid, typeid, lenheadid, lendataid, heade
     1rid, dataid
* variable shapes
      integer dims(3)
...
C     Jean-Michel Brankart (Oct. 94)
      ...
      SUBROUTINE TOPOS ( LAT , LON , LAT1 , LAT2 , LAT3 ,
     +                               LON1 , LON2 , LON3 )
