      Subroutine UWRIT2(iu,c8,c4,valex8,ipre8,imaxc,jmaxc,kmaxc,nbmots)
!c                ======
...
!c single precision
        if(iprec.eq.4) then
         do 10 kl=1,nl
          write(iu,ERR=99) (SNGL(c4(ide+kc)),kc=1,nbmots)
          ide=ide+nbmots
 10      continue
          write(iu,ERR=99) (SNGL(c4(ide+kc)),kc=1,ir)
                       else
!c
!c double precision
        if(iprec.eq.8) then
