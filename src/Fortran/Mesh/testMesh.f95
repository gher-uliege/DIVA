...
!C ----------------------------------------------------
 ...
 USE moduleChrono
      ...
      INTEGER(KIND=4) ::  NMAX,MMAX,NCMAX,NRMAX, i1

TYPE(Chronometer) :: chron, chron1

      PARAMETER(NMAX=4000000,MMAX=4000000,NCMAX=1000,NRMAX=100)
...
!C     NRMAX: NOMBRE MAXIMUM DE REGION A DENSITE VARIABLE
      ...
      REAL(KIND=8) ::  X(NMAX),Y(NMAX),NOEUD(NMAX,2),S
      REAL(KIND=8) ::  NEWPT(NMAX,3),XMAX,XMIN,YMAX,YMIN
      REAL(KIND=8) ::  L,DX,DY,NX,NY,C,XP,YP
      ...
      COMMON/CMEAN/XRMEAN,YRMEAN
      RPI=2*ASIN(1.)
      write(6,*) 'Pi:',RPI
      P=0
      
      CALL createDIVAContext()
      CALL startChrono(chron1)
      CALL startChrono(chron)
...
!C Now here, subtract mean coordinate position to enhance numerical
!C precusion on mesh-generation
      ...
      XRMEAN=XRMEAN/N
      YRMEAN=YRMEAN/N
      write(6,*) 'Now centering coordinates',XRMEAN,YRMEAN
      ...
      DO  I=1,NR
        DO  J=1,KNMAX(I)
           KN(Kk+J,1)=KN(Kk+J,1)-XRMEAN
           KN(Kk+J,2)=KN(Kk+J,2)-YRMEAN
        enddo
        Kk=Kk+KNMAX(I)
      enddo
      endif
...
!C     DETECTION DES MAILLES D'ENTR�E DE CONCAVITE
!Ctest
!c      CALL SORTIE(MAILLE,NOEUD,NMAX,MMAX,NO,MA)
!c      stop
!C???
...
!C     SORTIE DES RESULTATS DANS DES FICHIERS 20 ET 21
      ...
      IMPLICIT NONE
      INTEGER(KIND=4) ::  NMAX,MMAX,NO,MA,I,J,MAILLE(MMAX,6)
      REAL(KIND=8) ::  NOEUD(NMAX,2)
...
!C      INTRODUCTION
      ...
      IMPLICIT NONE
      INTEGER(KIND=4) ::  NPOURR,NPOURT
      PARAMETER(NPOURR=50000)
      PARAMETER(NPOURT=50000)
      INTEGER(KIND=4) ::  NMAX,MMAX,R(NPOURR),M,I,T(NPOURT),B(4),L,J,MAILLE(MMAX,6)
      REAL(KIND=8) ::  NOEUD(NMAX,2),XP,YP,xi
      REAL(KIND=8) ::  X1,Y1,Y2,X2,Z1,Z2,Z3,Z4,A1,A2,A3,C1
      INTEGER(KIND=4) ::  S,A(NPOURT),N,K,F,P,SW,IIII,IOLD

      REAL(KIND=8) ::  V,N1,N2,N3,N4,NX,NY,VOL,V2,V3
      REAL(KIND=8) ::  XJ1,YJ1,XJ2,YJ2,XJ3,YJ3,VJ,XJC,YJC
      NX=XP
      NY=YP
...
!C       INTRODUIT LE NOUVEAU NOEUD EN AJUSTANT LA TOPOLOGIE.
      INTEGER(KIND=4) ::  NPOURR,NPOURC
      PARAMETER(NPOURR=50000,NPOURC=50000)
      INTEGER(KIND=4) ::  NMAX,MMAX,MA,NO,R(NPOURR),NM,T,MAILLE(MMAX,6)
      REAL(KIND=8) ::  NOEUD(NMAX,2),XP,YP
      INTEGER(KIND=4) ::  I,J,K,L,CE(NPOURC,2),MTEMP(NPOURC,5),A,B,C
      INTEGER(KIND=4) ::  MV(NPOURC,2)
...
!C     SUPPRIME LA MAILLE NM DU TABLEAU
      ...
      INTEGER(KIND=4) ::  MMAX,MA,NM,I,J
      INTEGER(KIND=4) ::  MAILLE(MMAX,6)
...
!C      FACTEUR C PRES. 'C' EST LU DANS UN FICHIER 11
      ...
      IMPLICIT NONE
      INTEGER(KIND=4) ::  I,P,NRMAX,NR
      INTEGER(KIND=4) ::  NMAX,MMAX,MA,NO,KNMAX(NRMAX),MAILLE(MMAX,6)
!c      REAL(KIND=8) ::  NOEUD(NMAX,2),XP,YP,XM,YM,X1,Y1,X2,Y2,X3,Y3
!c      REAL(KIND=8) ::  A,B,C,D,E,F,AR,L,KN(NMAX,2),G(NRMAX),S
!CJMB
      REAL(KIND=8) ::  X1,X2,X3,Y1,Y2,Y3,B,D,E,F,A,XM,YM,AR
      REAL(KIND=8) ::  NOEUD(NMAX,2),XP,YP
      REAL(KIND=8) ::  C,L,KN(NMAX,2),G(NRMAX),S
...
!C
      ...
      INTEGER(KIND=4) ::  MMAX,MA,MAC,I,J,K,NT,NV,MAILLE(MMAX,6)
      INTEGER(KIND=4) ::  ATEST(jm1),LMA(jm2),N1,N2,E,F,M,NCMAX,PPTC(NCMAX),NC,L
...
!C       ON LE 'LISSERA' NL FOIS, OU NL EST LU DANS UN FICHIER 11
      ...
      INTEGER(KIND=4) ::  jm1
      parameter(jm1=5000)
      INTEGER(KIND=4) ::  NMAX,MMAX,MA,NO,B(4),C,A,MAILLE(MMAX,6)
      REAL(KIND=8) ::  NOEUD(NMAX,2),AX,AY
      INTEGER(KIND=4) ::  I,J,K,M,N,MN(jm1,2),NN(jm1),S,T,NL,P
      REAL(KIND=8) ::  NX,NY,V,N1,N2,N3,N4,D,V0,RR,NXOLD,NYOLD
...
!C      LA VALEUR DE L CORRESPONDANTE (INCHANGEE SI EN DEHORS D'UNE REGION)
      ...
      INTEGER(KIND=4) ::  I,J,T,M,N,NR,NRMAX,NMAX,KNMAX(NRMAX)
      REAL(KIND=8) ::  XP,YP,KN(NMAX,2),G(NRMAX),L
      REAL(KIND=8) ::  X1,X2,Y1,Y2,V,A,B
      M=0
      DO 10 I=1,NR 
       N=M
       M=M+KNMAX(I)
...
!C     RAJOUTE UN POINT SI DEUX POINTS DE CONTOUR NE SONT PAS RELIES
      ...
      INTEGER(KIND=4) ::  I,J,K,M,NO,MA,NMAX,NCMAX,MMAX,MAILLE(MMAX,6)
      INTEGER(KIND=4) ::  R,P,L,NC,PPTC(NCMAX),N,Q,MP,istop
      REAL(KIND=8) ::  NOEUD(NMAX,2),XP,YP
      INTEGER(KIND=4) ::  ichange
      ichange=0
 777  continue
      MP=1
      P=4
...
!C     A CE QUE LES NOEUD SOIENT DANS L'ORDRE
      ...
      INTEGER(KIND=4) ::  I,NO,MA,NMAX,MMAX,MAILLE(MMAX,6),M,J,K,S,L
      REAL(KIND=8) ::  NOEUD(NMAX,2),A,B
...
!C     along one integer array and one REAL(KIND=8) ::  array during the sort by
!C     Mark K. Seager
...
!C=============================================================================
      ...
      subroutine llxy(x,y)
      REAL(KIND=8) ::  rlatmin,rlatmax,rlonmean,rlatmean,dxkm,dykm,rpi
      REAL(KIND=8) ::  rcoordchange,x,y,xxx,dyyy
      integer isspheric, icoordchange
      COMMON/COORDCH/rlatmin,rlatmax,rlonmean,rlatmean,dxkm,dykm,RPI,rcoordchange,isspheric,icoordchange
      if(isspheric.eq.1)  then
