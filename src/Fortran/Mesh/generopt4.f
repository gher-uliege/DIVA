...
C     NRMAX: NOMBRE MAXIMUM DE REGION A DENSITE VARIABLE
      ...
      REAL*4 X(NMAX),Y(NMAX),NOEUD(NMAX,2),S
      REAL*4 NEWPT(NMAX,3),XMAX,XMIN,YMAX,YMIN
      REAL*4 L,DX,DY,NX,NY,C,XP,YP
      ...
      REAL*4 KN(NMAX,2),G(NRMAX)
      INTEGER*4 NTRI(NMAX)
      REAL*8 D
      include'iodv.h'
      real rlonmin,rlonmax
      real rlatmin,rlatmax,rlonmean,rlatmean,dxkm,dykm,rpi
      real rcoordchange
...
C      write(6,*) ' so what??',N
      ...
      Z1=1.D0*NOEUD(MAILLE(A(N),1),1)**2-NOEUD(MAILLE(A(N),2),1)**2 
      Z2=1.D0*NOEUD(MAILLE(A(N),1),2)**2-NOEUD(MAILLE(A(N),2),2)**2
      Z3=1.D0*NOEUD(MAILLE(A(N),2),2)**2-NOEUD(MAILLE(A(N),3),2)**2
      Z4=1.D0*NOEUD(MAILLE(A(N),2),1)**2-NOEUD(MAILLE(A(N),3),1)**2 
      IF(Y1.EQ.0) THEN
...
c      REAL*4 NOEUD(NMAX,2),XP,YP,XM,YM,X1,Y1,X2,Y2,X3,Y3
c      REAL*4 A,B,C,D,E,F,AR,L,KN(NMAX,2),G(NRMAX),S
CJMB
      REAL*8 X1,X2,X3,Y1,Y2,Y3,B,D,E,F,A,XM,YM,AR
      REAL*4 NOEUD(NMAX,2),XP,YP
      REAL*4 C,L,KN(NMAX,2),G(NRMAX),S
...
C       ON LE 'LISSERA' NL FOIS, OU NL EST LU DANS UN FICHIER 11
      ...
      REAL*4 NOEUD(NMAX,2),AX,AY
      INTEGER*4 I,J,K,M,N,MN(jm1,2),NN(jm1),S,T,NL,P
      REAL*8 NX,NY,V,N1,N2,N3,N4,D
...
C      LA VALEUR DE L CORRESPONDANTE (INCHANGEE SI EN DEHORS D'UNE REGION)
      ...
      REAL*4 XP,YP,KN(NMAX,2),G(NRMAX),L
...
C ---  MMAX: NOMBRE MAX DE MAILLES
      ...
      REAL*4 NOEUD(NMAX,2),X1,X2,Y1,Y2,X,Y
