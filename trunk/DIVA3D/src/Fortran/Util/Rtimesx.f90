         parameter(ndatamax=1E7)
         parameter(ndim=2)
         real*8 coord(ndim,ndatamax),x(ndatamax),Rx(ndatamax)
       ...
       rcoordchange,isspheric,icoordchange
        ...
        i=0
 1      continue
        read(20,*,end=999,err=999) coord(1,i+1),coord(2,i+1),x(i+1)
        i=i+1
        goto 1
 999    continue
        ndata=i
...
! Now implement coordinate change on the N points and Density regions if ncessessary
      ...
      icoordchange=0
      isspheric=0
      read(5,*,err=9911,end=9911) RL,rcoordchange
