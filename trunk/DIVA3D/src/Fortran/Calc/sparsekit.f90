...
!    Input, integer ( kind = 4 ) NCOL, the column dimension of the matrix.
!
!    Input, integer ( kind = 4 ) JOB, job indicator.  When JOB = 0, only the structure
...
!
!    Output, integer ( kind = 4 ) NDEGR(NROW), contains the degrees (the number of 
!    nonzeros in each row of the matrix A * B.
!
!    Output, integer ( kind = 4 ) NNZ, the number of nonzero elements found in A * B.
!
...
!
!    Input, integer ( kind = 4 ) NDIAG, the first dimension of array adiag as declared in
!    the calling program.
!
...
!
!    a, ja = the real and integer ( kind = 4 ) arrays of the Ellpack/Itpack format
...
!    the A matrix.
!
!    Input, integer ( kind = 4 ) JDIAG, the number of jagged-diagonals in the data
!    structure.
!
...
!
! ia     = integer ( kind = 4 ) array containing the lengths of the  jagged diagonals
!
!    Output, real Y(N), the product A*X.
!
  implicit none
...
!
!    Output, real A(*), integer ( kind = 4 ) JA(*), IA(NODE_NUM+1), the assembled global
...
!
!    Input, integer ( kind = 4 ) ELEMENT_NODE(NPE,ELEMENT_NUM), the connectivity matrix.
!    ELEMENT_NODE(I,J) is the global index of the I-th local node in element J.
!
!    Input, integer ( kind = 4 ) NODE_CODE(NODE_NUM), boundary information for each node 
...
!    each element.
!
!    Output, real A(*), integer ( kind = 4 ) JA(*), IA(NX+1), the assembled matrix in CSR
!    Compressed Sparse Row format.
!
...
!        resp. of the nodes.
!
! iwk,jwk = two integer ( kind = 4 ) work arrays.
...
!
! len   = integer ( kind = 4 ). length of arrays a and ja. bndcsr will stop if the
...
!    Input, integer ( kind = 4 ) NELX, the number of elements.
!
!    Input/output, integer ( kind = 4 ) IJK(NODE,NELX), lists the nodes that make up 
...
!         sparse row format.
!
! ja      = integer ( kind = 4 ) array of length n/nblk. ja(k) contains the column index
!         of the leading element, i.e., the element (1,1) of the block
!         that is held in the row a(k,*) of the value array.
!
! ia    = integer ( kind = 4 ) array of length n/nblk+1. ia(i) points to the beginning
...
!    in CSR format.
!
! ja      = integer ( kind = 4 ) array of length nnz containing the column positions
!         of the corresponding elements in a.
!
! ia      = integer ( kind = 4 ) array of length nnz containing the row positions
!         of the corresponding elements in a.
!
...
!    Input, integer ( kind = 4 ) NROW, the row dimension of the matrix.
!
!    Input, integer ( kind = 4 ) NNZ, the number of nonzero elements in the matrix.
!
! a,
! ir,
...
!
!    Input, integer ( kind = 4 ) NNZ, the number of nonzero elements in the sparse matrix.
!
!    Input, integer ( kind = 4 ) NCA, the first dimension of output arrays CA and JAC.
!
!  A(NNZ)  - Real array.
...
!            entries in A.
!
!  NER     - integer ( kind = 4 ). Must be set greater than or equal to the maximum
...
!    Compressed Sparse Row format.
!
!    Input, integer ( kind = 4 ) IPOS, indicates the position in the array AO, JAO
...
!         of the leading element, i.e., the element (1,1) of the block
!         that is held in the row a(k,*) of the value array.
! iao   = integer ( kind = 4 ) array of length n/nblk+1. ia(i) points to the beginning
!        of block row number i in the arrays a and ja.
!
  implicit none
...
!         of A on return
!
! ioff  = integer ( kind = 4 ) array of length idiag, containing the offsets of the
...
! work arrays:
!
! ind   = integer ( kind = 4 ) array of length 2*n-1 used as integer ( kind = 4 ) work space.
...
!
!    Output, real B(*), integer ( kind = 4 ) JB(*), integer ( kind = 4 ) IB(NROW+1), the resulting 
...
!    be at least NE.
!
!    Output, integer ( kind = 4 ) NE, the number of nonzero elements in the sparse matrix
...
!
!    Output, integer ( kind = 4 ) IA(NN), pointers to specify rows for the stored nonzero
!    entries in AR.
!
!    Output, integer ( kind = 4 ) JA(NN), pointers to specify columns for the stored 
!    nonzero entries in AR.
!
!    Output, integer ( kind = 4 ) IERR, an error parameter, returned as zero on successful
...
!
  ...
  real ( kind = 8 ) eps
  real ( kind = 8 ) errst
!JMB added (1,1) to cpmply with routine rank
  real ( kind = 8 ) hh(1,1)
  integer ( kind = 4 ) ierr
  integer ( kind = 4 ) ih
  integer ( kind = 4 ) indic
...
!
! ioff           = integer ( kind = 4 ) array containing the offsets  of the ndiag diagonals
...
!
!JMB2013 COMMENTED OUT, not used anywar
...
!    Compressed Sparse Row format.
!
!    Input, integer ( kind = 4 ) NBLK, the dimension of each diagonal block.  The diagonal
...
!
!    Output, integer ( kind = 4 ) IERR, error flag, which is nonzero if an error occurred.
!
  implicit none
...
subroutine pgmres ( n, im, rhs, sol, vv, eps, maxits, iout, &
  aa, ja, ia, alu, jlu, ju, ierr ,residue)
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, integer ( kind = 4 ) IM, the size of the Krylov subspace.  IM should not 
...
!    Compressed Sparse Row format.
!
! perm       = integer ( kind = 4 ) array of length nrow containing the permutation arrays
...
!                      used).
!
!    Output, real AO(*), integer ( kind = 4 ) JAO(*), IAO(NROW+1), the permuted matrix
!    in CSR Compressed Sparse Row format.
!
! note :
...
!
!    Input, real A(*), integer ( kind = 4 ) JA(*), IA(NCOL+1), the transposed matrix
!    in CSR Compressed Sparse Row format.
!
