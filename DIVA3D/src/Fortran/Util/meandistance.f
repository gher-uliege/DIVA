...
c        write(6,*) ' Into fitting '
 ...
 1      continue
        ...
        n=n+1
        if (n.ge.nm) then
        write(6,*) 'Open meandistance.f and increase parameter nm'
        stop
        endif
        goto 1
 99     continue
...
c        write(6,*) 'RL,SN,VARBAK',RL,SN,VARBAK
c        if (icoord.eq.1) then
c        write(66,*) 'Correlation length (in degrees latitude)'
c                         else
c        write(66,*) 'Correlation length'
c        endif
c        write(66,*) RL
c        write(66,*) 'Signal to noise ratio'
c        write(66,*) SN
c        write(66,*) 'VARBAK'
c        write(66,*) VARBAK
c        write(66,*) 'Quality of the fit (0: bad 1: good)'
c        write(66,*) rqual
c        if (icoord.ge.1) then
c        write(66,*) 'For information: correlation length in km is',
c     &      RL*6400.*3.14/180.
c        write(6,*) 'For information: correlation length in km is',
c     &      RL*6400.*3.14/180.
c
c             endif
        stop
...
c        write(6,*) 'Number of data points : ', n
c        write(6,*) 'data mean', datamean
c        write(6,*) 'data variance ',variance
...
c        write(6,*) 'Number of data couples considered',rjjj
        meandist=meandist/rjjj

        
                
c        write(6,*) 'maximum distance between points: ',maxdist
...
c        write(6,*) 'Number of probable active bins:',rnbins
        
        ddist=meandist/rnbins
        nbmax=maxdist/ddist+1
c        write(6,*) 'distance for binning: ',ddist
c        write(6,*) 'maximum number of bins: ',nbmax
        if(nsamp.eq.0) then
c        write(6,*) 'Average number of pairs in each bin', rn*rn/nbmax/2
        else
c        write(6,*) 'Average number of pairs in each bin', 
c     &      nsamp*nsamp/nbmax/2
        endif
        do ii=1,nbmax
        work(ii)=0
        w2(ii)=0
...
c        w3(nn)=exp(-(nn-10)**2/100))
 ...
 147    format(5(E14.6))
     ...
     & and.(nn.gt.4)) then
c        write(6,*) 'First zero crossing',nn,ddist,nn*ddist
...
c        write(6,*) 'Now trying to fit Bessel covariance function'
        errmin=1.E35
        VAR=variance
        RL=rlz
...
C Compute Mean Value
 ...
 20      CONTINUE
         ...
         B(2)=0
         B(3)=0
c         write(6,*) 'Using average value as reference',B(1)
         ENDIF
